...
class RouterController {
 public:
  ...
  int delete_route(uint32_t prefix, int pLen, uint32_t nhop, uint8_t tos, uint16_t port);
  int add_drop_flow(uint32_t prefix, int pLen);
 ...
 protected:
  ...
  int delete_route_(uint32_t prefix, int pLen, uint32_t nhop, uint8_t tos, uint16_t port, UpdateMode udpate_mode);
