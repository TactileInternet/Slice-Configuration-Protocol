...
int RouterController::add_drop_flow(uint32_t prefix, int pLen, uint8_t tos) {
  int rc = 0;
...
int RouterController::delete_route_(uint32_t prefix, int pLen, uint8_t tos, UpdateMode update_mode) {
  int rc = 0;
  ...
  if (update_mode == UpdateMode::DEVICE_STATE) {
    ...
    mf_e->set_field_id(pi_p4info_table_match_field_id_from_name(p4info, t_id, "hdr.ipv4.tos"));
...
int RouterController::delete_route(uint32_t prefix, int pLen, uint8_t tos) {

  int rc = 0;
  rc |= delete_route_(prefix, pLen, tos, UpdateMode::CONTROLLER_STATE);
  rc |= delete_route_(prefix, pLen, tos, UpdateMode::DEVICE_STATE);
  return rc;
}
...
int RouterController::static_config_(UpdateMode update_mode) {
  ...
  int if_num = 1;
  unsigned char hw[6]; 
  //the first interface is the local controller
  string_to_mac("bb:bb:bb:bb:bb:bb", hw);
  add_iface_(if_num++, (uint32_t)boost::asio::ip::address_v4::from_string("0.0.0.0").to_ulong(), hw, update_mode);
...
void Networkgraph::makeSourceRoute(std::string route, RouterController* rec, uint32_t dst_addr, uint8_t tos){ 
  std::vector<std::string> tokens;
  unsigned int rt[17];
  char port = 0;
  unsigned char rt_c[16];
  boost::split(tokens, route, boost::is_any_of("-"));
  for (unsigned int i = 0 ; i < 17; i++) {
      unsigned int x;
      std::stringstream ss;
      if (i< tokens.size()-1) ss << std::hex << tokens[i];
      else ss << std::hex << "fe";
      ss >> x;
      rt[i]=x;
      if (i == 0) port = (char)rt[i];
      else {
        rt_c[i-1]=(char)rt[i];
      }
  } 
  rec->installSourceRoute(rt_c, port, dst_addr, tos, tokens.size());
}

void RouterController::installSourceRoute(const unsigned char (&sourcRoute)[16], int port, uint32_t dst_addr, uint8_t tos, int length){
  pi_p4_id_t t_id = pi_p4info_table_id_from_name(p4info, "slice_sw");
  pi_p4_id_t a_id = pi_p4info_action_id_from_name(p4info, "source_ingress");

    p4v1::TableEntry match_action_entry;
    match_action_entry.set_table_id(t_id);

    auto mf = match_action_entry.add_match();
    mf->set_field_id(pi_p4info_table_match_field_id_from_name(p4info, t_id, "hdr.ipv4.dstAddr"));
    auto mf_lpm = mf->mutable_lpm();
    mf_lpm->set_value(uint_to_string(dst_addr));// & 0xFFFFFF00));
    mf_lpm->set_prefix_len(32);
    //uint8_t tos2 = 1;
    auto mf_e = match_action_entry.add_match();
    mf_e->set_field_id(pi_p4info_table_match_field_id_from_name(p4info, t_id, "hdr.ipv4.tos"));
    auto mf_exact = mf_e->mutable_exact();
    ...
    {
      auto param = action->add_params();
      param->set_param_id(pi_p4info_action_param_id_from_name(p4info, a_id, "sourceRoute"));
      param->set_value(std::string(reinterpret_cast<const char *>(sourcRoute), sizeof(sourcRoute)));

    }
    {
      auto param = action->add_params();
      param->set_param_id(pi_p4info_action_param_id_from_name(p4info, a_id, "port"));
      param->set_value(uint_to_string(static_cast<uint16_t>(port)));
    }
    {
      auto param = action->add_params();
      param->set_param_id(pi_p4info_action_param_id_from_name(p4info, a_id, "type"));
      param->set_value(uint_to_string(static_cast<uint16_t>(1)));
    }
    {
      auto param = action->add_params();
      param->set_param_id(pi_p4info_action_param_id_from_name(p4info, a_id, "length"));
      param->set_value(uint_to_string(static_cast<uint16_t>(length-1)));
    }
    if(add_one_entry(&match_action_entry)) std::cout << "Error when adding entry to ipv4_lpm (source route rule)\n";
}
...
void Networkgraph::getRoute(int src, int dst, uint32_t dst_addr, uint8_t tos, uint16_t reason, RouterController* rec, std::string weight, int numFlow, bool tactile_flow){
  ...
  // Extract the shortest path from src to dst. Weight is latency.
  path_t path;
  ...
  if (tos != 0) {
  	std::cout<<"\tDistance:"<<totalDist<<" Limit:"<< latency[tos]<<" \n";
  	if (totalDist >= latency[tos]) {
  	  	std::cout<<predecessors[v]<<"Connection not possible! The flow will be blocked.";
		rec->add_drop_flow(dst_addr & 0xffffff00, 24, tos);	//block
		return;
	  }
  }
  ...
  for(path_t::reverse_iterator riter = path.rbegin(); riter != path.rend(); ++riter) {
    ...
    } else if (reason == TACTILE_FLOW) {
	srcRoute += std::to_string(g[e_tmp].interfaces[0].second) + "-";
    }
 
  } 
  if(reason == TACTILE_FLOW) {
	std::cout<<"TACTILE_FLOW CPU packet received!!"<<std::endl;
	this->makeSourceRoute(srcRoute, rec, dst_addr, tos); //+std::string(1,'1')
  ...
  for (;;) {
    ...
    for(unsigned int i = 0; i < getNumberRouters(); i++) {
      ...
      for (auto &it : router->ifaces ){
        usleep(10);
        if (it.port_num != 1) router->send_lldp_packet(it);
      }
    }
